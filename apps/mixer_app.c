...
static void mixer_draw_slider(int x, int y, int w, uint8_t val) {
    ...
    uint32_t thumb = app_blend_color(MIXER_THUMB, 0xFFFFFFu,
                                     (uint32_t)app_anim_pingpong(100u, 12), 12u);

    app_draw_rect(x, y, w, MIXER_TRACK_H, MIXER_TRACK);
    app_draw_rect(thumb_x, y - ((MIXER_THUMB_H - MIXER_TRACK_H) / 2), MIXER_THUMB_W, MIXER_THUMB_H, thumb);
}
...
static void mixer_update_hover(void) {
    ...
    for (idx = 0; idx < 5; idx++) {
        ...
        if (app_point_in_rect(local_x, local_y, x, y, w, h)) {
...
static void mixer_on_draw(int win_x, int win_y, int win_w, int win_h) {
    ...
    app_draw_string(MIXER_PADDING, MIXER_PADDING, "Master", MIXER_LABEL_COLOR, MIXER_BG);
    {
        ...
        label[0] = '\0';
        app_append_u32(label, (uint32_t)sizeof(label), mixer_master_volume);
    ...
    for (idx = 0; idx < (int)AUDIO_MAX_SOURCES; idx++) {
        ...
        if (audio_is_playing(idx)) {
            int level = 8 + app_anim_pingpong(36u + (uint32_t)(idx * 6), vh - 8);
            app_draw_rect(vx, vy, vw, vh, MIXER_VU_OFF);
            app_draw_rect(vx, vy + vh - level, vw, level, MIXER_VU_ON);
        } else {
            app_draw_rect(vx, vy, vw, vh, MIXER_VU_OFF);
        }
    }
...
static void mixer_on_click(int x, int y, int btn) {
    ...
    for (idx = 0; idx < (int)AUDIO_MAX_SOURCES; idx++) {
        ...
        mixer_mute_rect(idx, &mx, &my, &mw, &mh);
        if (app_point_in_rect(x, y, mx, my, mw, mh)) {
    ...
    for (idx = 0; idx < 5; idx++) {
        ...
        mixer_test_button_rect(idx, &bx, &by, &bw, &bh);
        if (app_point_in_rect(x, y, bx, by, bw, bh)) {
            mixer_play_test(idx);
            return;
        }
    }

    {
        ...
        mixer_slider_rect(MIXER_DRAG_MASTER, &sx, &sy, &sw, &sh);
        if (app_point_in_rect(x, y, sx, sy - 6, sw, sh + 12)) {
